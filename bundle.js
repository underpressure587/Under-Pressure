...
async function _boot() {
  ...
  if (window.GSPAuth) {
    (async () => {
      ...
      // 1o: verifica sessao local (rapido)
      try {
      ...
      // 2o: verifica retorno do redirect Google (so se nao tem sessao local)
      try {
        const redir = await window.GSPAuth.processarRedirectGoogle();
        if (redir) { await _loginOk(redir); return; }
      } catch(e) {}
    })();
  }
}
