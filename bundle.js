...
async function _boot() {
  ...
  // Mostra login imediatamente sem esperar Firebase
  mostrarTela('screen-login');

  // Verifica sessao ativa em background
  if (window.GSPAuth) {
    (async () => {
      let t = 0;
      while (!window.GSPAuth.isReady() && t < 20) {
        await new Promise(r => setTimeout(r, 100));
        t++;
      }
      if (!window.GSPAuth.isReady()) return;

      // Verifica retorno do redirect Google E sessao local em paralelo
      try {
        const [redir, fbUser] = await Promise.all([
          window.GSPAuth.processarRedirectGoogle().catch(() => null),
          window.GSPAuth.waitForAuthReady().catch(() => null),
        ]);
        const user = redir || (fbUser ? {
          uid: fbUser.uid,
          nome: fbUser.displayName || fbUser.email?.split('@')[0] || 'Jogador',
          email: fbUser.email,
          tipo: 'user'
        } : null);
        if (user) { await _loginOk(user); return; }
      } catch(e) {}
    })();
  }
}
...
function _registrarResultado(score, scoreGestor, sector, companyName) {
  alert('DEBUG: uid=' + _player?.uid + ' tipo=' + _player?.tipo + ' GSPSync=' + !!window.GSPSync + ' GSPAuth=' + !!window.GSPAuth);
  ...
  if (!isGuest && _player?.uid) {
    const _salvarNoFirestore = () => {
      ...
      const _timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 60000));
      Promise.race([
        Promise.all([
