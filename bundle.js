...
async function _boot() {
  ...
  // ── FAST PATH: se já tem dados locais, entra imediatamente sem loading ──
  ...
  // Usuário novo ou sem sessão — mostra login imediatamente
  mostrarTela('screen-login');

  // Verifica em background se tem sessão ativa (ex: retorno do Google)
  if (window.GSPAuth) {
    (async () => {
      let t = 0;
...
async async function authGoogle() {
  if (!window.GSPAuth?.isReady()) {
