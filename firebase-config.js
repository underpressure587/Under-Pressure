...
// Sinaliza que firebase-config.js terminou de carregar
...
window.GSPAuth = {
  ...
  async loginGoogle() {
    if (!_firebaseReady) throw new Error("Firebase não configurado.");
    // Sempre usa redirect — mais confiável no mobile e evita bloqueio de popup
  ...
  async _salvarPerfil(user, nome) {
    if (!db) {
      alert("ERRO: db não inicializado em _salvarPerfil");
      return;
    }
    try {
      ...
      alert("ERRO _salvarPerfil: " + e.code + " - " + e.message);
      console.warn("[GSP] _salvarPerfil:", e.message);
    }
  },
};
...
window.GSPSync = {
  ...
  async salvarPartida(uid, entrada) {
    if (!db || !uid) { alert('ERRO salvarPartida: db=' + !!db + ' uid=' + uid); throw new Error('db/uid ausente'); }
    try {
    const result = await addDoc(collection(db, 'usuarios', uid, 'historico'), {
    ...
    return result;
      ...
      alert('ERRO ESCRITA: ' + e.code + ' | ' + e.message);
      throw e;
    }
  },
